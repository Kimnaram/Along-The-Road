...
#        echo $escaped_value;
		...
		try {
			...
			if ($stmt->rowCount() == 0) {
				...
				$hotel_sql="select hotel_name from users where uid='$uid'";
				$hstmt = $con->prepare($hotel_sql);
				$hstmt->execute();
				
				if ($hstmt->rowCount() == 0) {
					$stmt = $con->prepare("UPDATE plan SET start_date=:start_date, end_date=:end_date, stay=:stay, hotel_name=:hotel_name, image=:image, url=:url WHERE uid='$uid'");
					$stmt->bindParam(':uid', $uid);
			  		$stmt->bindParam(':city', $city);
			  		$stmt->bindParam(':start_date', $start_date);
			  		$stmt->bindParam(':end_date', $end_date);
